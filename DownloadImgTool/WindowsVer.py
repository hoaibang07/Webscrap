...
__copyright__ = "Copyright 2007, HBC"
__license__ = "GPL"
__version__ = "1.0.1"
    ...
    try:
        ...
        for image in soup.findAll("img"):
            ...
            if sourceImg.find('.gif') == -1:
                ...
                print "Adding image to IDM queue..."

                #send link to idm queue
                if sourceImg.startswith("http"):
                    sendLinkToIDM(sourceImg, filename, output_folder)
                else:
                    sendLinkToIDM(urlparse.urlunparse(parsed), filename, output_folder)

        if imgCount == 0:
            print "No image found!"
            sys.exit(-1)
        if imgCount == 1:
            print "Completed! Added %d image to IDM queue" %imgCount
        else:
            print "Completed! Added %d images to IDM queue" %imgCount
...
# removeAccents of string - Vietnamese
def removeAccents(s):
...
def sendLinkToIDM(url, filename, output_folder):
    programFileLocation = os.environ['PROGRAMFILES']
    IDM = programFileLocation + "\\Internet Download Manager\\IDMan /d " + fixurl(url) + " /n /p "\
    + output_folder + "\\" + " /f " + removeAccents(u"" + filename) + " /a"
    #start adding to IDM queue
    subprocess.call(IDM)
